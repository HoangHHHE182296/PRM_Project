import 'package:test/test.dart';
import 'package:internal_openapi/internal_openapi.dart';

// tests for CategorySummaryResponse
void main() {
  final instance = CategorySummaryResponseBuilder();
  // TODO add properties to the builder and call build()

  group(CategorySummaryResponse, () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // String imageUrl
    test('to test the property `imageUrl`', () async {
      // TODO
    });

    // ProductType supportedProductType
    test('to test the property `supportedProductType`', () async {
      // TODO
    });

    // String parentCategoryId
    test('to test the property `parentCategoryId`', () async {
      // TODO
    });

    // String parentCategoryName
    test('to test the property `parentCategoryName`', () async {
      // TODO
    });

  });
}
